...
let s:css_data = eval(join(readfile(expand("<sfile>:p:h") . '/../dict/css-data.json')))
let s:css_props = get(s:css_data, "properties")
...
function! csscomplete#CompleteCSS(findstart, base)
  ...
  if len(borders) == 0 || borders[max(keys(borders))] =~ '^\%(openbrace\|semicolon\|opencomm\|closecomm\|style\)$'
    ...
    for m in s:css_props
      let s:prop_name = get(m, 'name')
      let postfix = ''
      if strridx(after, ':') < 0
        let postfix = ': '
      endif
      if s:prop_name =~? '^'.entered_property
        call add(res, {'word': s:prop_name . postfix, 'info': get(m, 'description')})
      elseif s:prop_name =~? entered_property
        call add(res2, {'word': s:prop_name . postfix, 'info': get(m, 'description')})
      endif
    endfor

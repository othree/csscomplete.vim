...
let s:props_info = {
      \   "all": {
      \     "info": "Shorthand that resets all properties except 'direction' and 'unicode-bidi'.",
      \   },
      \ }
...
function! csscomplete#CompleteCSS(findstart, base)
  ...
  if len(borders) == 0 || borders[max(keys(borders))] =~ '^\%(openbrace\|semicolon\|opencomm\|closecomm\|style\)$'
    ...
    for m in keys(s:props_info)
      let postfix = ''
      if strridx(after, ':') < 0
        let postfix = ': '
      endif
      if m =~? '^'.entered_property
        call add(res, {'word': m . postfix, 'info': get(get(s:props_info, m), 'info')})
      elseif m =~? entered_property
        call add(res2, {'word': m . postfix, 'info': get(get(s:props_info, m), 'info')})
      endif
    endfor
